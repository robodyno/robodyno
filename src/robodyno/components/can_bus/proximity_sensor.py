...
The proximity sensor can detect whether there is a metal object approaching.

Examples:
    >>> from robodyno.components import ProximitySensor
    >>> from robodyno.interfaces import CanBus
    >>> can = CanBus()
    >>> proximity_sensor = ProximitySensor(can)
...
class ProximitySensor(CanBusDevice):
    """Proximity sensor driver
    ...
    _CMD_GET_COSWSRNSOR_DATA = 0x04

    def __init__(self, can: CanBus, id_: int = 0x36):
        """Initializes the proximity sensor driver.
        Args:
        ...
        if self.type is None or self.type != Model.ROBODYNO_PROXIMITY_SENSOR:
            raise ValueError(f'Device of id {id_} is not a proximity sensor driver.')
    ...
    def get_data(self, timeout: Optional[float] = 0.01) -> Optional[int]:
